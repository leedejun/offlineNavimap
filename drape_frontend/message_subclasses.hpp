...
namespace df
{
    ...
    class DrapeApiAddPolygonsMessage : public Message
    {
    public:
        explicit DrapeApiAddPolygonsMessage(DrapeApi::TPolygons const & polygons)
                : m_polygons(polygons)
        {}

        Type GetType() const override { return Type::DrapeApiAddPolygons; }

        DrapeApi::TPolygons const & GetPolygons() const { return m_polygons; }

    private:
        DrapeApi::TPolygons m_polygons;
    };

    class DrapeApiAddCustomMarksMessage : public Message
    {
    public:
        explicit DrapeApiAddCustomMarksMessage(DrapeApi::TCustomMarks const & marks)
                : m_marks(marks)
        {}

        Type GetType() const override { return Type::DrapeApiAddCustomMarks; }

        DrapeApi::TCustomMarks const & GetCustomMarks() const { return m_marks; }

    private:
        DrapeApi::TCustomMarks m_marks;
    };
