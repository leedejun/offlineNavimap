...
public class StringUtils
{
    ...
    public static String getFileSizeString(long size)
    {
        if (size < Constants.GB)
        {
            ...
            return String.format(Locale.US, "%1$d %2$s", value, MapTestApplication.get().getString(R.string.mb));
        }

        float value = ((float)size / Constants.GB);
        return String.format(Locale.US, "%1$.1f %2$s", value, MapTestApplication.get().getString(R.string.gb));
    }
