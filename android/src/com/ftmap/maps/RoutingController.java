...
import com.ftmap.util.ConnectionState;
import com.ftmap.util.StringUtils;
...
//        void onTaxiError(@NonNull TaxiManager.ErrorCode code);
    ...
    private final Logger LOGGER = LoggerFactory.INSTANCE.getLogger(LoggerFactory.Type.ROUTING);
    @Nullable
    ...
    private int mWaitingPoiPickType = RoutePointInfo.ROUTE_MARK_START;
    ...
    {
        mCachedRoutingInfo = FTMap.nativeGetRouteFollowingInfo();
        if (mLastRouterType == Framework.ROUTER_TYPE_TRANSIT)
            mCachedTransitRouteInfo = FTMap.nativeGetTransitRouteInfo();
    ...
    {
        mLastRouterType = FTMap.nativeGetLastUsedRouter();
        mInvalidRoutePointsTransactionId = FTMap.nativeInvalidRoutePointsTransactionId();
        mRemovingIntermediatePointsTransactionId = mInvalidRoutePointsTransactionId;

        FTMap.nativeSetRoutingListener(mRoutingListener);
        FTMap.nativeSetRouteProgressListener(mRoutingProgressListener);
        FTMap.nativeSetRoutingRecommendationListener(mRoutingRecommendationListener);
        FTMap.nativeSetRoutingLoadPointsListener(mRoutingLoadPointsListener);
    }
...
//                        Statistics.EventParam.TO, Statistics.getPointType(getEndPoint())});
                                ...
                                final boolean fromApi)
    {
        ...
        for (int resId : new int[] { R.string.dialog_routing_disclaimer_priority, R.string.dialog_routing_disclaimer_precision,
            ...
            builder.append(MapTestApplication.get().getString(resId)).append("\n\n");
    ...
    {
        FTMap.nativeLoadRoutePoints();
    }

    public boolean hasSavedRoute()
    {
        return FTMap.nativeHasSavedRoutePoints();
    }

    public void saveRoute()
    {
        if (isNavigating() || (isPlanning() && isBuilt()))
            FTMap.nativeSaveRoutePoints();
    }
                                   ...
                                   boolean fromApi)
    {
        ...
        if (startPoint != null && endPoint != null)
            mLastRouterType = FTMap.nativeGetBestRouter(startPoint.getLat(), startPoint.getLon(),
                        ...
                        @Framework.RouterType int routerType, boolean fromApi)
    {
        ...
        FTMap.nativeSetRouter(mLastRouterType);
...
//            AlohaHelper.logClick(AlohaHelper.ROUTING_START_SUGGEST_REBUILD);
    ...
    public void removeStop(@NonNull MapObject mapObject)
    {
        ...
        FTMap.nativeRemoveRoutePoint(info.mMarkType, info.mIntermediateIndex);
    ...
    {
        FTMap.nativeRemoveIntermediateRoutePoints();
    }
    ...
    {
        return FTMap.nativeCouldAddIntermediatePoint() && !isTaxiRouterType();
    }
    ...
    {
        RouteMarkData[] points = FTMap.nativeGetRoutePoints();
    ...
    private static void addRoutePoint(@RoutePointInfo.RouteMarkType int type, @NonNull MapObject point)
    {
        ...
        FTMap.nativeAddRoutePoint(description.first /* title */, description.second /* subtitle */,
    ...
    private static Pair<String, String> getDescriptionForPoint(@NonNull MapObject point)
    {
        ...
        else
        {
            ...
            {
                title = FTMap.nativeFormatLatLon(point.getLat(), point.getLon(), false /* useDmsFormat */);
            }
        }
...
//        AlohaHelper.logClick(AlohaHelper.ROUTING_SWAP_POINTS);
    ...
    private void openRemovingIntermediatePointsTransaction()
    {
        if (mRemovingIntermediatePointsTransactionId == mInvalidRoutePointsTransactionId)
            mRemovingIntermediatePointsTransactionId = FTMap.nativeOpenRoutePointsTransaction();
    }
    ...
    private void cancelRemovingIntermediatePointsTransaction()
    {
        if (mRemovingIntermediatePointsTransactionId == mInvalidRoutePointsTransactionId)
            return;
        FTMap.nativeCancelRoutePointsTransaction(mRemovingIntermediatePointsTransactionId);
        mRemovingIntermediatePointsTransactionId = mInvalidRoutePointsTransactionId;
    }
    ...
    private void applyRemovingIntermediatePointsTransaction()
    {
        ...
        if (mRemovingIntermediatePointsTransactionId == mInvalidRoutePointsTransactionId)
            return;
        FTMap.nativeApplyRoutePointsTransaction(mRemovingIntermediatePointsTransactionId);
        mRemovingIntermediatePointsTransactionId = mInvalidRoutePointsTransactionId;
    }
