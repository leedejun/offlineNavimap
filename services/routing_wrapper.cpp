...
void GetAllLocalFiles(vector<LocalCountryFile> & localFiles,Config & config)
{
  string resourDir;
  string writableDirForTests;  
  config.readConfigFile("resourDir",resourDir);
  config.readConfigFile("writableDirForTests",writableDirForTests);
...
//    pl.SetResourceDir(options.m_resourcePath);
  pl.SetResourceDir(resourDir);
  pl.SetWritableDirForTests(writableDirForTests);
...
shared_ptr<VehicleRouterComponents> CreateAllMapsComponents(VehicleType vehicleType,Config & config)
{
  vector<LocalCountryFile> localFiles;
  GetAllLocalFiles(localFiles,config);
  ASSERT(!localFiles.empty(), ());
  return make_shared<VehicleRouterComponents>(localFiles, vehicleType);
}

IRouterComponents & GetVehicleComponents(VehicleType vehicleType,Config & config)
{
  ...
  if (it == kVehicleComponents.end())
    tie(it, ignore) = kVehicleComponents.emplace(vehicleType, CreateAllMapsComponents(vehicleType,config));
...
TRouteResult CalculateRoute(IRouterComponents const & routerComponents,
                            Checkpoints const & checkpoints, GuidesTracks && guides,vector<Polygon> const & polygons)
{
  RouterDelegate delegate;
  shared_ptr<Route> route = make_shared<Route>("mapsme", 0 /* route id */);
  //routerComponents.GetRouter().SetGuides(move(guides));
  routerComponents.GetRouter().SetAvoidPolygon(polygons);
  RouterResultCode result = routerComponents.GetRouter().CalculateRoute(
