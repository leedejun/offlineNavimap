...
public:
  ...
  std::shared_ptr<FeaturesFetcher> const & GetFeaturesFetcher() const noexcept { return m_featuresFetcher; }
  std::unique_ptr<storage::CountryInfoGetter> const & GetInfoGetter() const noexcept { return m_infoGetter; }
...
std::shared_ptr<VehicleRouterComponents> CreateAllMapsComponents(VehicleType vehicleType,conf::Config & config);
IRouterComponents & GetVehicleComponents(VehicleType vehicleType,conf::Config & config);
...
TRouteResult CalculateRoute(IRouterComponents const & routerComponents,
                            Checkpoints const & checkpoints, GuidesTracks && guides,std::vector<Polygon> const & polygons);
