...
FeatureType::FeatureType(osm::MapObject const &emo) {
    ...
    switch (emo.GetGeomType()) {
        ...
        case feature::GeomType::Area:
            ...
            m_triangles = Points(emo.GetTriangesAsPoints().begin(),
                                 emo.GetTriangesAsPoints().end());
...
void FeatureType::ParseTypes() {
    ...
    try {
        ...
        LOG(LERROR,
            ("Incorrect type index for feature.FeatureID:", m_id, ". Incorrect index:", index,
...
uint32_t FeatureType::ParseGeometry(int scale) {
    ...
    if (!m_parsed.m_points) {
        ...
        auto const headerGeomType = static_cast<HeaderGeomType>(Header(m_data) &
                                                                HEADER_MASK_GEOMTYPE);
        if (headerGeomType == HeaderGeomType::Line) {
