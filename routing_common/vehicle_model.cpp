...
bool VehicleModel::IsPassThroughAllowed(FeatureType & f) const
{
  ...
  for(int i = 0; i < f.GetPointsCount(); i++){
    auto p = f.GetPoint(i);
    boost::geometry::append ( line, Point(p.x, p.y) );
  }

  bool isIntersection = false;
  for(auto it:m_polygon){
    std::vector<Point> result;
    boost::geometry::intersection(line,it,result);
...
bool VehicleModelFactory::SetAvoidPolygon(vector<Polygon> const & polygons) const{
  vector<Polygon> p = polygons;
  m_polygon.swap(p);
  return true;
}
