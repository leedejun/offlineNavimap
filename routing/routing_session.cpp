...
namespace routing {
    ...
    RoutingSession::RoutingSession()
            : m_router(nullptr),
              ...
              m_state(SessionState::NoValidRoute), m_isFollowing(false),
              m_speedCameraManager(m_turnNotificationsMgr),
              m_routingSettings(GetRoutingSettings(VehicleType::Car)),
              m_passedDistanceOnRouteMeters(0.0), m_lastCompletionPercent(0.0) {
...
//  alohalytics::LogEvent("OnRoutingInit", params);
    ...
    void RoutingSession::BuildRoute(Checkpoints const &checkpoints, GuidesTracks &&guides,
        ...
        RebuildRoute(checkpoints.GetStart(), m_buildReadyCallback, m_buildReadyCallbackKsp,
                     m_needMoreMapsCallback,
                     m_removeRouteCallback, timeoutSec, SessionState::RouteBuilding,
                     false /* adjust */);
    }
    ...
    void RoutingSession::RebuildRoute(m2::PointD const &startPoint,
        ...
        m_router->CalculateRoute(checkpoints, direction, adjustToPrevRoute,
                                 DoReadyCallback(*this, readyCallback, readyCallbackKsp),
                                 DoReadyCallbackKsp(*this, readyCallback, readyCallbackKsp),
                                 needMoreMapsCallback,
                                 removeRouteCallback, m_progressCallback, timeoutSec);
    }

    void RoutingSession::BuildRoute2(Checkpoints const &checkpoints, ReadyCallbackKsp const &readyCallback,
                                NeedMoreMapsCallback const &needMoreMapsCallback,
                                RemoveRouteCallback const &removeRouteCallback, uint32_t requireRouteCount) {
        SetState(SessionState::RouteBuilding);
        ++m_routingRebuildCount;
        auto const &direction = m_routingSettings.m_useDirectionForRouteBuilding
                                ? m_positionAccumulator.GetDirection()
                                : m2::PointD::Zero();
        ...
        m_router->ClearState();
        m_router->CalculateRoute2(checkpoints, direction, false,
                                  DoReadyCallbackKsp(*this, ReadyCallback(), readyCallback), needMoreMapsCallback,
                             removeRouteCallback, m_progressCallback, requireRouteCount, routing::RouterDelegate::kNoTimeout);
    }
    ...
    void RoutingSession::DoReadyCallbackKsp::operator()(shared_ptr<Route> route, RouterResultCode e) {
        ASSERT(m_rs.m_route, ());
        m_rs.AssignRoute(route, e);
        m_callback(*m_rs.m_route, e);
    }

    void RoutingSession::DoReadyCallbackKsp::operator()(const vector<shared_ptr<Route>>& vec_route,
                                                        RouterResultCode e) {
        ASSERT(m_rs.m_route, ());
//        for (int i = 0; i < 3; i++) {
//            m_rs.AssignRoute(vec_route[i], e);
    ...
    void RoutingSession::RebuildRouteOnTrafficUpdate() {
...

        {
            ...
            switch (m_state) {
                ...
                case SessionState::RouteFinished:
                    return;
                ...
                case SessionState::RouteNeedRebuild:
                    break;
            }
        ...
        RebuildRoute(startPoint, m_rebuildReadyCallback, m_buildReadyCallbackKsp,
...
//      alohalytics::LogEvent("RouteTracking_ReachedDestination", params);
        ...
        if (m_state != SessionState::RouteNeedRebuild && m_state != SessionState::RouteRebuilding) {
            ...
            double const dist = mercator::DistanceOnEarth(lastGoodPoint,
                                                          mercator::FromLatLon(info.m_latitude,
                                                                               info.m_longitude));
            if (base::AlmostEqualAbs(dist, m_lastDistance, kRunawayDistanceSensitivityMeters))
                return m_state;

            if (!info.HasSpeed() ||
                info.m_speedMpS < m_routingSettings.m_minSpeedForRouteRebuildMpS)
                m_moveAwayCounter += 1;
            else
                m_moveAwayCounter += 2;
...
//                                            mercator::XToLon(lastGoodPoint.x)));
    ...
    void RoutingSession::GetRouteFollowingInfo(FollowingInfo &info) const {
        ...
        if (!IsNavigable()) {
            info = FollowingInfo();
            FormatDistance(m_route->GetTotalDistanceMeters(), info.m_distToTarget,
                           info.m_targetUnitsSuffix);
            info.m_time = static_cast<int>(max(kMinimumETASec, m_route->GetCurrentTimeToEndSec()));
            return;
        }

        FormatDistance(m_route->GetCurrentDistanceToEndMeters(), info.m_distToTarget,
                       info.m_targetUnitsSuffix);
        ...
        info.m_pedestrianTurn =
                (distanceToTurnMeters < kShowPedestrianTurnInMeters) ? turn.m_pedestrianTurn
...
//        alohalytics::Location::FromLatLon(lastGoodPoint.m_lat, lastGoodPoint.m_lon));
    ...
    void RoutingSession::PassCheckpoints() {
        CHECK_THREAD_CHECKER(m_threadChecker, ());
        while (!m_checkpoints.IsFinished() &&
               m_route->IsSubroutePassed(m_checkpoints.GetPassedIdx())) {
        ...
        if (m_proj.m_edge.GetFeatureId() == proj.m_edge.GetFeatureId()) {
            if (m_route->GetCurrentRoutingSettings().m_matchRoute) {
                if (!base::AlmostEqualAbs(m_proj.m_point, proj.m_point, kEps)) {
                    location.m_bearing =
                            location::AngleToBearing(
                                    base::RadToDeg(ang::AngleTo(m_proj.m_point, proj.m_point)));
                }
            }
    ...
    void RoutingSession::SetRoutingCallbacks(ReadyCallback const &buildReadyCallback,
        ...
        CHECK_THREAD_CHECKER(m_threadChecker, ());
        m_buildReadyCallback = buildReadyCallback;
        m_buildReadyCallbackKsp = buildReadyCallbackKsp;
        m_rebuildReadyCallback = rebuildReadyCallback;
        m_needMoreMapsCallback = needMoreMapsCallback;
        m_removeRouteCallback = removeRouteCallback;
    }
...
//      alohalytics::Location::FromLatLon(lastGoodPoint.m_lat, lastGoodPoint.m_lon));
    ...
    void RoutingSession::SetLocaleWithJsonForTesting(std::string const &json,
