    ...
    namespace {
        map<string, string> PrepareStatisticsData(string const &routerName,
            ...
            return {{"name",            routerName},
                    {"startLon",        strings::to_string_dac(mercator::XToLon(startPoint.x),
                                                               precision)},
                    {"startLat",        strings::to_string_dac(mercator::YToLat(startPoint.y),
                                                               precision)},
                    ...
                    {"finalLon",        strings::to_string_dac(mercator::XToLon(finalPoint.x),
                                                               precision)},
                    {"finalLat",        strings::to_string_dac(mercator::YToLat(finalPoint.y),
                                                               precision)}};
        }

        void SendStatistics(m2::PointD const &startPoint, m2::PointD const &startDirection,
                            m2::PointD const &finalPoint, RouterResultCode resultCode,
                            double routeLenM,
                            double elapsedSec,
            ...
            map<string, string> statistics = PrepareStatisticsData(routerName, startPoint,
                                                                   startDirection, finalPoint);
...
// ----------------------------------------------------------------------------------------------------------------------------
    ...
    AsyncRouter::RouterDelegateProxy::RouterDelegateProxy(ReadyCallbackOwnership const &onReady,
                                                          ...
                                                          uint32_t timeoutSec)
            : m_onReadyOwnership(onReady), m_onReadyOwnershipKsp(onReadyKsp),
              m_onNeedMoreMaps(onNeedMoreMaps),
              m_onRemoveRoute(onRemoveRoute), m_onPointCheck(onPointCheck),
    ...
    AsyncRouter::RouterDelegateProxy::RouterDelegateProxy(ReadyCallbackOwnership const &onReady,
                                                          ...
                                                          ProgressCallback const &onProgress,
                                                          uint32_t timeoutSec)
            : m_onReadyOwnership(onReady), m_onNeedMoreMaps(onNeedMoreMaps),
              m_onRemoveRoute(onRemoveRoute), m_onPointCheck(onPointCheck),
              m_onProgress(onProgress) {
        m_delegate.Reset();
        m_delegate.SetPointCheckCallback(bind(&RouterDelegateProxy::OnPointCheck, this, _1));
    ...
    void AsyncRouter::RouterDelegateProxy::OnReady(shared_ptr<Route> route,
    ...
    void AsyncRouter::RouterDelegateProxy::OnReady(std::vector<std::shared_ptr<Route>> route,
                                                   RouterResultCode resultCode) {
        if (!m_onReadyOwnershipKsp)
            return;
        {
            lock_guard<mutex> l(m_guard);
            if (m_delegate.IsCancelled())
        ...
        m_onReadyOwnershipKsp(move(route), resultCode);
    }
    //  added by wangzd,
...
// -------------------------------------------------------------------------------------------------
    ...
    void
    AsyncRouter::SetRouter(unique_ptr<IRouter> &&router, unique_ptr<IOnlineFetcher> &&fetcher) {
        unique_lock<mutex> ul(m_guard);
    ...
    void AsyncRouter::CalculateRoute(Checkpoints const &checkpoints, m2::PointD const &direction,
        ...
        m_delegateProxy =
                make_shared<RouterDelegateProxy>(readyCallback, needMoreMapsCallback,
                                                 removeRouteCallback,
                                                 m_pointCheckCallback, progressCallback,
                                                 timeoutSec);
    ...
    void AsyncRouter::CalculateRoute(Checkpoints const &checkpoints, m2::PointD const &direction,
                                     bool adjustToPrevRoute,
                                     ReadyCallbackOwnership const &readyCallback,
                                     ReadyCallbackOwnershipKsp const &readyCallbackKsp,
                                     NeedMoreMapsCallback const &needMoreMapsCallback,
                                     RemoveRouteCallback const &removeRouteCallback,
                                     ProgressCallback const &progressCallback,
                                     uint32_t timeoutSec) {
        ...
        m_delegateProxy =
                make_shared<RouterDelegateProxy>(readyCallback, readyCallbackKsp,
                                                 needMoreMapsCallback,
                                                 removeRouteCallback,
                                                 m_pointCheckCallback, progressCallback,
                                                 timeoutSec);
    ...
    void AsyncRouter::CalculateRoute2(Checkpoints const & checkpoints, m2::PointD const & direction,
    ...
    {
        unique_lock<mutex> ul(m_guard);
        m_requireRountCount = requireRouteCount;
        m_checkpoints = checkpoints;
        m_startDirection = direction;
        m_adjustToPrevRoute = adjustToPrevRoute;
        ...
        m_delegateProxy =
                make_shared<RouterDelegateProxy>(ReadyCallbackOwnership(), readyCallbackKsp,
                                                 needMoreMapsCallback,
                                                 removeRouteCallback,
                                                 m_pointCheckCallback, progressCallback,
                                                 timeoutSec);
    ...
    void AsyncRouter::LogCode(RouterResultCode code, double const elapsedSec) {
        switch (code) {
            ...
            case RouterResultCode::TransitRouteNotFoundNoNetwork:
                LOG(LWARNING,
                    ("No transit route is found because there's no transit network in the mwm of "
                     "the route point"));
                break;
    ...
    void AsyncRouter::ThreadFunc() {
        while (true) {
            {
                unique_lock<mutex> ul(m_guard);
                m_threadCondVar.wait(ul, [this]() {
                    return m_threadExit || m_hasRequest || m_clearState;
                });
...
//    LOG(LINFO, ("ETA:", route->GetTotalTimeSec(), "sec."));
            ...
            GetPlatform().RunTask(Platform::Thread::Gui, [checkpoints, startDirection, e,
                ...
                SendStatistics(checkpoints.GetStart(), startDirection, checkpoints.GetFinish(),
                               e.Msg(),
                               routingStatisticsCallback, routerName);
            });
            ...
            //得到三条线路，画三条线
//    for(int i=0; i<3; i++){
//        auto item=vec_route[i];
            GetPlatform().RunTask(Platform::Thread::Gui,
                                  [delegateProxy, vec_route, code]() {
                                      delegateProxy->OnReady(vec_route, code);
                                  });
...
//  double const routeLengthM = vec_route[0]->GetTotalDistanceMeters();
//  GetPlatform().RunTask(
...
//          GetPlatform().RunTask(Platform::Thread::Gui,
//                                [delegateProxy, vec_route, code]() { delegateProxy->OnReady(vec_route[2], code); });
            if( m_requireRountCount > 1 ){
                GetPlatform().RunTask(Platform::Thread::Gui,
                                      [delegateProxy, vec_route, code]() {
                                          delegateProxy->OnReady(vec_route, code);
                                      });
                ...
                auto item = vec_route[0];
                GetPlatform().RunTask(Platform::Thread::Gui,
                                      [delegateProxy, item, code]() {
                                          delegateProxy->OnReady(item, code);
                                      });
            }
        }
        ...
        absent.insert(vec_route[2]->GetAbsentCountries().cbegin(),
                      vec_route[2]->GetAbsentCountries().cend());
        if (!absent.empty())
        ...
        if (code != RouterResultCode::NoError) {
            if (code == RouterResultCode::NeedMoreMaps) {
                ...
                GetPlatform().RunTask(Platform::Thread::Gui,
                                      [delegateProxy, code]() {
                                          delegateProxy->OnRemoveRoute(code);
                                      });
            }
        }
    }
