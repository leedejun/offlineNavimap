...
public:
  ...
  struct SubrouteSettings final
  {
    ...
    std::vector<search::Result> const m_POIResult;
  };
  ...
  std::vector<search::Result> const & GetViaPOI() const { return m_POIResult; }
  ...
  inline void SetOnTheWayPOI(std::vector<search::Result> POIResult)
  {
...
private:
  ...
  //on the way poi
  std::vector<search::Result> m_POIResult;
};
} // namespace routing
