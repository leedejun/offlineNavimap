...
SearchAPI::SearchAPI(DataSource &dataSource, storage::Storage const &storage,
                     ...
                     Delegate &delegate)
        : m_dataSource(dataSource), m_storage(storage), m_infoGetter(infoGetter),
          m_delegate(delegate), m_engine(m_dataSource, GetDefaultCategories(), m_infoGetter,
                                         Engine::Params(languages::GetCurrentTwine() /* locale */,
                                                        numThreads)) {
}
...
//  p.m_position =m2::Point(params.m_lat,params.m_lon);
...
bool SearchAPI::SearchInDownloader(storage::DownloaderSearchParams const &params) {
    ...
    p.m_onResults = DownloaderSearchCallback(
            static_cast<DownloaderSearchCallback::Delegate &>(*this),
