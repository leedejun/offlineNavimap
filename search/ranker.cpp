...
namespace search {
    namespace {
        ...
        template<typename Slice>
        void
        UpdateNameScores(vector<strings::UniString> const &tokens, uint8_t lang, Slice const &slice,
                         NameScores &bestScores) {
...
// This function supports only street names like "abcdstrasse"/"abcd strasse".
        vector<vector<strings::UniString>>
        ModifyStrasse(vector<strings::UniString> const &streetTokens) {
            ...
            auto const name =
                    strings::UniString(streetTokens.back().begin(),
                                       streetTokens.back().end() - kStrasse.size());
        ...
        pair<NameScores, size_t> GetNameScores(FeatureType &ft, Geocoder::Params const &params,
            ...
            if (type == Model::TYPE_BUILDING)
                UpdateNameScores(ft.GetHouseNumber(), StringUtf8Multilang::kDefaultCode,
                                 sliceNoCategories,
                                 bestScores);
            ...
            if (ftypes::IsAirportChecker::Instance()(ft)) {
                ...
                if (!iata.empty())
                    UpdateNameScores(iata, StringUtf8Multilang::kDefaultCode, sliceNoCategories,
                                     bestScores);
            }

            string const op = ft.GetMetadata().Get(feature::Metadata::FMD_OPERATOR);
            if (!op.empty())
                UpdateNameScores(op, StringUtf8Multilang::kDefaultCode, sliceNoCategories,
                                 bestScores);
            ...
            if (type == Model::TYPE_STREET) {
                for (auto const &shield : feature::GetRoadShieldsNames(ft.GetRoadNumber()))
                    UpdateNameScores(shield, StringUtf8Multilang::kDefaultCode, sliceNoCategories,
                                     bestScores);
            }
        ...
        void
        MatchTokenRange(FeatureType &ft, Geocoder::Params const &params, TokenRange const &range,
            ...
            for (auto const token : range) {
                errorsMade += ErrorsMade{
                        GetMaxErrorsForToken(params.GetToken(token).GetOriginal())};
            ...
            auto equalCmp = [&p, &minDistanceOnMapBetweenResults](RankerResult const &r) -> bool {
                if (p.GetResultType() == r.GetResultType() &&
                    ...
                    if (p.IsEqualCommon(r))
                        return PointDistance(p.GetCenter(), r.GetCenter()) <
                               minDistanceOnMapBetweenResults;
                }
...
//            auto const &pivot = m_ranker.m_params.m_accuratePivotCenter;
            ...
            if (m_params.IsCategorialRequest()) {
                ...
                if (!info.m_hasName) {
                    ...
                    info.m_hasName =
                            meta.Has(feature::Metadata::FMD_OPERATOR) ||
                            meta.Has(feature::Metadata::FMD_BRAND);
                }
            } else {
                auto const scores = GetNameScores(ft, m_params, preInfo.InnermostTokenRange(),
                                                  info.m_type);
                ...
                if (info.m_type != Model::TYPE_SUBURB &&
                    ...
                    if (suburb) {
                        ...
                        MatchTokenRange(*suburb, m_params, range, type, suburbErrors,
                                        suburbMatchedLength,
                ...
                info.m_matchedFraction =
                        totalLength == 0 ? 1.0
                                         : static_cast<double>(matchedLength) /
                ...
                auto const isCountryOrCapital = [](FeatureType &ft) {
                    ...
                    auto static const capitalType = classif().GetTypeByPath(
                ...
                info.m_exactCountryOrCapital =
                        info.m_errorsMade == ErrorsMade(0) && info.m_allTokensUsed &&
            ...
            CategoriesInfo const categoriesInfo(feature::TypesHolder(ft),
                                                ...
                                                m_ranker.m_params.m_categoryLocales,
                                                m_ranker.m_categories);
    ...
    public:
        RankerResultMaker(Ranker &ranker, DataSource const &dataSource,
                ...
                : m_ranker(ranker), m_dataSource(dataSource), m_params(params),
...
//            116.39126521640469  43.59062286906408  天安门
            ...
            info.m_rank = NormalizeRank(info.m_rank, info.m_type, center, country,
                                        ftypes::IsCapitalChecker::Instance()(*ft),
    ...
    Ranker::Ranker(DataSource const &dataSource, CitiesBoundariesTable const &boundariesTable,
            ...
            : m_reverseGeocoder(dataSource), m_cancellable(cancellable),
              m_keywordsScorer(keywordsScorer),
              m_localities(dataSource, boundariesTable, villagesCache), m_dataSource(dataSource),
              m_infoGetter(infoGetter), m_emitter(emitter), m_categories(categories),
              m_suggests(suggests) {
        SetLocale("default");
    }
    ...
    Result Ranker::MakeResult(RankerResult const &rankerResult, bool needAddress,
        ...
        auto mk = [&](RankerResult const &r) -> Result {
            switch (r.GetResultType()) {
                ...
                case RankerResult::Type::LatLon:
                    return Result(r.GetCenter(), name, address);
                case RankerResult::Type::Postcode:
                    return Result(r.GetCenter(), name);
            }
    ...
    void Ranker::UpdateResults(bool lastUpdate) {
        ...
        for (; i < m_tentativeResults.size(); ++i)
        {
            ...
            //tujingdianguolv
            if(m_params.m_line.size()>0){
                double distance = boost::geometry::distance(Point(result.GetCenter().x,result.GetCenter().y),m_params.m_line);
                if(distance>3){
                    continue;
                }
            }
